...
import { Mic, Target, Users, BarChart3, Bell, Menu, X, Settings, Search, LayoutDashboard, FileText, UserCircle } from 'lucide-react';
...
export const Layout: React.FC<LayoutProps> = ({ currentView, setView, children }) => {
      ...
      return (
          <div className="min-h-screen bg-gradient-to-br from-pink-50 to-orange-50 font-sans p-6">
              <header className="flex justify-between items-center max-w-6xl mx-auto mb-8">
                  ...
                  <div className="text-sm font-bold text-slate-400">Step 1/4</div>
              </header>
              <main>
                  {children}
              </main>
  ...
  const NavItem = ({ target, icon: Icon, label }: { target: ViewState; icon: React.ElementType; label: string }) => (
    <button
      ...
      className={`w-full flex items-center gap-3 px-6 py-4 rounded-full transition-all duration-300 group relative overflow-hidden ${
        currentView === target 
    ...
    <div className="flex h-screen bg-transparent overflow-hidden font-sans">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-white/60 backdrop-blur-2xl border-r border-white/50 text-slate-800 transform transition-transform duration-300 ease-out lg:relative lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} flex flex-col shadow-[4px_0_24px_rgba(0,0,0,0.02)]`}>
        <div className="p-8">
          ...
          <nav className="space-y-2">
            <NavItem target="REPORT" icon={UserCircle} label="Il mio Profilo" />
            <NavItem target="VOICE_CV" icon={Mic} label="Voice CV" />
            <NavItem target="SKILL_MATRIX" icon={Target} label="Skill Matrix" />
            <NavItem target="EMPLOYABILITY" icon={BarChart3} label="Employability" />
            <NavItem target="JOB_MATCHING" icon={Users} label="Job Matching" />
          </nav>
        </div>

        <div className="mt-auto p-6">
             <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-3xl p-6 text-white shadow-xl relative overflow-hidden group cursor-pointer hover:scale-105 transition-transform">
                ...
                <p className="text-[10px] font-bold uppercase tracking-widest mb-2 text-slate-400">Potenziale</p>
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative">
        ...
        <header className="lg:hidden bg-white/60 backdrop-blur-md border-b border-slate-100 p-4 flex justify-between items-center z-40 sticky top-0">
           ...
           <button onClick={() => setSidebarOpen(true)} className="text-slate-600 p-2 hover:bg-slate-100 rounded-lg transition-colors">
             <Menu />
           </button>
        </header>
        ...
        <div className="hidden lg:flex justify-between items-center px-10 py-6 sticky top-0 z-30">
           <div className="flex flex-col">
              <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Area Riservata</span>
              <h2 className="text-2xl font-extrabold text-slate-800 tracking-tight">
                  {currentView === 'REPORT' ? 'Sintesi del Profilo' : 
                   currentView === 'VOICE_CV' ? 'Aggiorna CV Vocale' :
                   currentView === 'JOB_MATCHING' ? 'AI Recruiting' : 'Dashboard'}
              </h2>
           </div>
           ...
           <div className="flex items-center gap-6">
              <div className="relative group">
                  ...
                  <input 
                    ...
                    className="pl-12 pr-4 py-3 bg-white border-none rounded-full text-sm shadow-sm focus:ring-2 focus:ring-pink-500 w-64 transition-all placeholder:text-slate-400 hover:shadow-md"
                  />
              </div>

              <button className="p-3 text-slate-400 hover:text-pink-500 relative bg-white hover:bg-pink-50 rounded-full transition-all shadow-sm hover:shadow-md">
                 <Bell size={20} />
                 <span className="absolute top-2.5 right-3 w-2 h-2 bg-pink-500 rounded-full ring-2 ring-white"></span>
              </button>
              
              <div className="flex items-center gap-3 pl-2 cursor-pointer" onClick={() => setView('REPORT')}>
